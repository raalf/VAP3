...
for i = 1:INPU.valVEHICLES
    ...
    if COND.valTRIMSTEP == 1
        if OUTP.vecVEHCM(i) > 0
            deltaEPS = 2*pi/180; % Deflect h-tail TE 2 deg down
            ...
            deltaEPS = -2*pi/180; % Deflect h-tail TE 2 deg up
            SURF.vecTAILPITCHold = SURF.vecDVEPITCH(SURF.vecDVEWING == 2)-COND.vecVEHALPHA(i)*pi/180;
            OUTP.vecVEHCMold(i) = OUTP.vecVEHCM(i);
        ...
        % Compute new h-tail incidence angle per vehicle
        tempS = (SURF.vecDVEPITCH(SURF.vecWINGTYPE == 2)-COND.vecVEHALPHA(i)*pi/180) -...
            OUTP.vecVEHCM*((SURF.vecDVEPITCH(SURF.vecWINGTYPE == 2)-COND.vecVEHALPHA(i)*pi/180)...
            - SURF.vecTAILPITCHold)/(OUTP.vecVEHCM(i)-OUTP.vecVEHCMold(i));
        
        OUTP.vecVEHCMold(i) = OUTP.vecVEHCM(i);
        
        deltaEPS = tempS - (SURF.vecDVEPITCH(SURF.vecWINGTYPE == 2)-COND.vecVEHALPHA(i)*pi/180);
