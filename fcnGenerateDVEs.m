% function [Aircraft, FW] = q(Aircraft, FW, ii)
...
% % %   V3 - Function complete overhaul for VAP2.0
% % 
...
X1 = matGEOM(1,1,n); Y1 = matGEOM(1,2,n); Z1 = matGEOM(1,3,n); 
rchord = matGEOM(1,4,n); repsilon = deg2rad(matGEOM(1,5,n));

X2 = matGEOM(2,1,n); Y2 = matGEOM(2,2,n); Z2 = matGEOM(2,3,n); 
tchord = matGEOM(2,4,n); tepsilon = deg2rad(matGEOM(2,5,n));
...
CP = reshape([PX2(2:2:end,2:2:end),PY2(2:2:end,2:2:end),PZ2(2:2:end,2:2:end)],vecM(n),vecN(n),3);
CP_Right = reshape([PX2(2:2:end,3:2:end),PY2(2:2:end,3:2:end),PZ2(2:2:end,3:2:end)],vecM(n),vecN(n),3);
LE_Mid = reshape([PX2(1:2:end-1,2:2:end),PY2(1:2:end-1,2:2:end),PZ2(1:2:end-1,2:2:end)],vecM(n),vecN(n),3);
TE_Right = reshape([PX2(3:2:end,3:2:end),PY2(3:2:end,3:2:end),PZ2(3:2:end,3:2:end)],vecM(n),vecN(n),3);
TE_Left = reshape([PX2(3:2:end,1:2:end-2),PY2(3:2:end,1:2:end-2),PZ2(3:2:end,1:2:end-2)],vecM(n),vecN(n),3);

% Filter Leading Edge Point
LE_Left = reshape([PX2(1:2:end-2,1:2:end-2),PY2(1:2:end-2,1:2:end-2),PZ2(1:2:end-2,1:2:end-2)],vecM(n),vecN(n),3);
LE_Right = reshape([PX2(1:2:end-2,3:2:end),PY2(1:2:end-2,3:2:end),PZ2(1:2:end-2,3:2:end)],vecM(n),vecN(n),3);
...
xsi_local = fcnGLOBSTAR3D( xsi_vec,nu,epsilon,zeros(vecM(n),vecN(n)) );
