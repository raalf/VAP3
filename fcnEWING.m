...
col1 = reshape([repmat((idx3.*5)-4,1,5) + repmat([0:4], len,1)]',[],1);
col2 = reshape([repmat((idx5.*5)-4,1,5) + repmat([0:4], len,1)]',[],1);
...
col1 = reshape([repmat((d2202.*5)-4,1,5) + repmat([0:4], len3,1)]',[],1);
col2 = reshape([repmat((d2204.*5)-4,1,5) + repmat([0:4], len3,1)]',[],1);
rows = reshape(repmat([1:len3]',1,5)',[],1);
...
col2 = reshape([repmat((edge3.*5)-4,1,5) + repmat([0:4], length(edge3),1)]',[],1);
col = [col1; col2];
rows = reshape(repmat([1:(length(edge1) + length(edge3))]',1,5)',[],1);
