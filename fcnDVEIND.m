function [a, b, c] = fcnDVEIND(dvenum, dvetype, fpg, vecK, matDVE, matVLST, matCENTER, vecDVEROLL, vecDVEPITCH, vecDVEYAW, vecDVELESWP, vecDVETESWP)
...
[a1le(idx1,:), b1le(idx1,:), c1le(idx1,:)] = fcnBOUNDIND(endpoints(idx1,:,:), vecDVELESWP(dvenum(idx1)), xsiA(idx1,:));


%% Vortex sheet at leading edge

[a2le, b2le, c2le] = fcnVSIND(endpoints, vecDVELESWP(dvenum), xsiA, vecK(dvenum));
...
[a1te(idx2,:), b1te(idx2,:), c1te(idx2,:)] = fcnBOUNDIND(endpoints(idx2,:,:), vecDVETESWP(dvenum(idx2)), xsiA(idx2,:));
...
[a2te, b2te, c2te] = fcnVSIND(endpoints(idx3,:,:), vecDVETESWP(dvenum(idx3)), xsiA(idx3,:), vecK(dvenum(idx3)));
