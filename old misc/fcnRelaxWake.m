...
for h = 2:Temp.timestep+1 % loop through timesteps (for indexing, we start at 1 instead of 0. Starting at 2 to ignore timestep 0)
    jj = 1;
    for i = 1:Aircraft.General.Panels
        ...
        for j = 1:length(FW.Panels(i).WakeDVE(h).Index)
            ...
            Temp.wDVEleft(h-1).w_ind(jj,:) = fcnDVEInducedVelocity(FW, Aircraft, Temp, P);
            % Troubleshooting issue with singfct/split tips/wake explosions
                        if i > 1
                            Temp.wDVEleft(h-1).w_ind(jj,:) = [0 0 0];
                        end
            Temp.wDVEleft(h-1).Panel(jj) = i;
            Temp.wDVEleft(h-1).sIndex(jj) = j;
    ...
    for j = 1:length(FW.Joint)
        if length(FW.Joint(j).Panel) == 1
            ...
            % split-tips
            Temp.uright(h-1).w_ind(jj,:) = fcnDVEInducedVelocity(FW, Aircraft, Temp, P);
            Temp.uright(h-1).Panel(jj) = FW.Joint(j).Panel;
            Temp.uright(h-1).sIndex(jj) = jj;
            Temp.uright(h-1).Wing(jj) = FW.Panels(FW.Joint(j).Panel).Wing;
            jj = jj + 1;
...
for h = 2:Temp.timestep+1
    ...
    if h == 2 % Second oldest row of wakeDVEs (Only current and upstream ??)
        for j = 1:length(Temp.wDVEleft(h-1).w_ind(:,1))
            P = FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:);
            %fprintf('xleft before: %f %f %f\n', P(1), P(2), P(3));
            FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:) = fcnDisplacePoint(FW, Temp.wDVEleft(h-1).w_ind(j,:), Temp.wDVEleft(h-1).w_ind(j,:), Temp.wDVEleft(h-1).w_ind(j,:), P);
            %             FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:)
        end
        
        % displacing wakeDVE wingtip edges
        for j = 1:length(Temp.uright(h-1).Panel)
            P = FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:);
            FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:) = fcnDisplacePoint(FW, Temp.uright(h-1).w_ind(j,:), Temp.uright(h-1).w_ind(j,:), Temp.uright(h-1).w_ind(j,:), P);
            %             FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:)
        end
    ...
    elseif h == Temp.timestep+1 % Youngest row of wakeDVEs (no upstream elements, only current and downstream)
        ...
        for j = 1:length(Temp.wDVEleft(h-1).w_ind(:,1))
            P = FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:);
            %fprintf('xleft before: %f %f %f\n', P(1), P(2), P(3));
            FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:) = fcnDisplacePoint(FW, Temp.wDVEleft(h-1).w_ind(j,:), [0 0 0], Temp.wDVEleft(h-2).w_ind(j,:), P);
            %FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:)
            %             fprintf('\nxLEFT after 2: %f %f %f', FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:));
        ...
        for j = 1:length(Temp.uright(h-1).Panel)
            P = FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:);
            FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:) = fcnDisplacePoint(FW, Temp.uright(h-1).w_ind(j,:), [0 0 0], Temp.uright(h-2).w_ind(j,:), P);
            %             FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:)
        end
    else
        % Displacing xleft points for all wakeDVEs
        for j = 1:length(Temp.wDVEleft(h-1).w_ind(:,1))
            P = FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:);
            %fprintf('xleft before: %f %f %f\n', P(1), P(2), P(3));
            FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:) = fcnDisplacePoint(FW, Temp.wDVEleft(h-2).w_ind(j,:), Temp.wDVEleft(h-1).w_ind(j,:), Temp.wDVEleft(h).w_ind(j,:), P);
            %             FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xright(Temp.wDVEleft(h-1).sIndex(j-1),:) =
            %FW.Panels(Temp.wDVEleft(h-1).Panel(j)).WakeDVE(h).xleft(Temp.wDVEleft(h-1).sIndex(j),:)
        end
        ...
        for j = 1:length(Temp.uright(h-1).Panel)
            P = FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:);
            FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:) = fcnDisplacePoint(FW, Temp.uright(h-2).w_ind(j,:), Temp.uright(h-1).w_ind(j,:), Temp.uright(h).w_ind(j,:), P);
            %             FW.Panels(Temp.uright(h-1).Panel(j)).WakeDVE(h).xright(end,:)
        end
    end
end
...
for h = Temp.timestep+1:-1:2
    for i = 1:Aircraft.General.Panels
        for j = 1:length(FW.Panels(i).WakeDVE(h).Index)
            ...
            elseif j == length(FW.Panels(i).WakeDVE(h).Index)
                ...
                for mm = 1:length(FW.Joint)
                    ...
                    elseif length(FW.Joint(mm).Panel) == 3 % If its a split in the wing (333 case)
                        ...
                        % Finding if this current panel i exists in this 333 joint
                        [indx1,indx2] = find(FW.Joint(mm).Panel == i);
                        
                        if ~isempty(indx1) && FW.Joint(mm).Edge(indx1(1)) == 2 % If it exists and is the right edge
                            [indx,~] = find(FW.Joint(mm).Panel ~= i); % Find the slots of the two other panels (not i)
                            FW.Panels(i).WakeDVE(h).xright(j,:) = FW.Panels(FW.Joint(mm).Panel(indx(1))).WakeDVE(h).xleft(1,:); % Force the right edge of current panel i to the left edge of one of the other panels
                            FW.Panels(FW.Joint(mm).Panel(indx(2))).WakeDVE(h).xleft(1,:) = FW.Panels(FW.Joint(mm).Panel(indx(1))).WakeDVE(h).xleft(1,:); % force the third panel's left edge to the same point (Not sure about this!)
                        end
            ...
            %             fprintf('\nNew XO: %f %f %f', FW.Panels(i).WakeDVE(h).xo(j,:));
            ...
            if h == Temp.timestep+1
                ...
                %fprintf('\nphiLE newest: %f',FW.Panels(i).WakeDVE(h).phiLE(j));
                ...
                %                     fprintf('\nphiLE oldest: %f',FW.Panels(i).WakeDVE(h).phiLE(j));
                ...
                [FW.Panels(i).WakeDVE(h).norm(j,:), FW.Panels(i).WakeDVE(h).roll(j), FW.Panels(i).WakeDVE(h).pitch(j), FW.Panels(i).WakeDVE(h).yaw(j), ...
                    ...
                    FW.Panels(i).WakeDVE(h).eta(j)] = fcnNewEtaNuEpsPsiXsi(Temp, US_DVE, FW.Panels(i).WakeDVE(h).xo(j,:), FW.Panels(i).WakeDVE(h).norm(j,:), ...
                    FW.Panels(i).WakeDVE(h).phiLE(j), FW.Panels(i).WakeDVE(h).phiMID(j), FW.Panels(i).WakeDVE(h).phiTE(j));
                %fprintf('\nphiLE else: %f',FW.Panels(i).WakeDVE(h).phiLE(j));
            end
