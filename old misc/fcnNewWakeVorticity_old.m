    ...
    for i = 1:Aircraft.General.Panels % going through all panels
        ...
        for ii = 1:FW.Panels(i).n % going through all spanwise wakeDVEs per panel
            ...
            if ii < FW.Panels(i).n; % Only performing this if we aren't at the very outter row of DVEs
                ...
                D_wake(row,cols) = [FW.Panels(i).WakeDVE(h).eta(ii) (2/3)*FW.Panels(i).WakeDVE(h).eta(ii)^2 FW.Panels(i).WakeDVE(h).eta(ii) -(2/3)*FW.Panels(i).WakeDVE(h).eta(ii)^2];
...
for j = 1:length(FW.Joint)
    ...
    if length(FW.Joint(j).Panel) == 1
        ...
        if FW.Joint(j).Edge == 2 % If the right edge is the one with the condition
            ...
            cols = 2*indx-1:2*indx; % Columns corresponding to the DVE elements
            D_wake(row,cols) = [etaSign(FW.Joint(j).Edge)*FW.Panels(FW.Joint(j).Panel).WakeDVE(h).eta(end) (2/3)*FW.Panels(FW.Joint(j).Panel).WakeDVE(h).eta(end)^2];
            R_wake(row) = -etaSign(FW.Joint(j).Edge)*FW.Panels(FW.Joint(j).Panel).WakeDVE(Temp.timestep+1 ).K(end);
        else % Otherwise it must be the left
            indx = FW.Panels(FW.Joint(j).Panel).WakeDVE(h).Index(1); % If not Edge 2, must be Edge 1
            cols = 2*indx-1:2*indx; % Columns corresponding to the DVE elements
            D_wake(row,cols) = [etaSign(FW.Joint(j).Edge)*FW.Panels(FW.Joint(j).Panel).WakeDVE(h).eta(1) (2/3)*FW.Panels(FW.Joint(j).Panel).WakeDVE(h).eta(1)^2];
            R_wake(row) = etaSign(FW.Joint(j).Edge)*FW.Panels(FW.Joint(j).Panel).WakeDVE(Temp.timestep+1 ).K(1);
        end
    ...
    elseif length(FW.Joint(j).Panel) == 2
        ...
        if FW.Joint(j).Edge(ed) == 2 % If the right edge is the one with the condition
            ...
            cols = 2*indx-1:2*indx; % Columns corresponding to the DVE elements
            D_wake(row,cols) = [FW.Joint(j).Edge(ed)*FW.Panels(FW.Joint(j).Panel(ed)).WakeDVE(h).eta(end) (2/3)*FW.Panels(FW.Joint(j).Panel(ed)).WakeDVE(h).eta(end)^2];
            D_wake(row+1,cols) = [FW.Joint(j).Edge(ed)*1 2*FW.Panels(FW.Joint(j).Panel(ed)).WakeDVE(h).eta(end)];
            ...
            cols = 2*indx-1:2*indx; % Columns corresponding to the DVE elements
            D_wake(row,cols) = [-FW.Joint(j).Edge(ed)*FW.Panels(FW.Joint(j).Panel(ed)).WakeDVE(h).eta(1) (2/3)*FW.Panels(FW.Joint(j).Panel(ed)).WakeDVE(h).eta(1)^2];
            D_wake(row+1,cols) = [FW.Joint(j).Edge(ed)*1 -2*FW.Panels(FW.Joint(j).Panel(ed)).WakeDVE(h).eta(1)];
