function [matWCOEFF] = fcnSOLVEWD_OL(matWD, vecWR, valWNELE, vecWKGAM, vecWDVEHVSPN, vecWDVEHVCRD)
...
matWCOEFF(:,2:5) = reshape(matWD\vecWR,4,valWNELE,1)';

matWCOEFF(isnan(matWCOEFF)) = 0

matWCOEFF(:,1) = (vecWKGAM - (1/3).*matWCOEFF(:,3).*(vecWDVEHVSPN.^2)) + (vecWKGAM - (1/3).*matWCOEFF(:,5).*(vecWDVEHVCRD.^2));
