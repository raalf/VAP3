...
OUTP.vecPREQ = nan(INPU.valVEHICLES,1);
OUTP.vecLD = nan(INPU.valVEHICLES,1);
...
if FLAG.VISCOUS == 1
    for i = 1:INPU.valVEHICLES
        ...
        if any(idxvehwing) && valTIMESTEP == COND.valMAXTIME
            ...
            [OUTP.vecCLv(i), OUTP.vecCD(i), OUTP.vecPREQ(i), OUTP.vecLD(i), OUTP.vecVINF(i)] = fcnVISCOUS_WING(OUTP.vecCL(end), OUTP.vecCDI(end), ...
        ...
        if any(idxvehrotor)
            if valTIMESTEP > COND.valMAXTIME - 1/(min(COND.vecROTORRPM)*COND.valDELTIME/60) % Only compute for last full rotor rotation
                ...
                [matROTORDP(idxvehrotor,:), vecDELNDIST(idxvehrotor)] = fcnVISCOUS_ROTOR(VISC.valKINV,...
                    SURF.vecDVEHVCRD(idxvehrotor==1), INPU.vecN, INPU.vecM, SURF.vecDVELE(idxvehrotor==1), SURF.vecDVEPANEL(idxvehrotor==1), VISC.cellAIRFOIL, SURF.vecDVENFREE(idxvehrotor==1)+SURF.vecDVENIND(idxvehrotor==1), SURF.vecDVEAREA(idxvehrotor==1),SURF.matUINF(idxvehrotor==1,:), SURF.matVLST, SURF.matDVE(idxvehrotor==1,:), matWUINF);
            end 
        end
    end 
end
