        ...
        if any(idxvehrotor)
            if valTIMESTEP > valMAXTIME - 1/(min(vecROTORRPM)*valDELTIME/60) % Only compute for last full rotor rotation
                ...
                [matROTORCDP(idxvehrotor,:)] = fcnVISCOUS_ROTOR(flagVERBOSE, ...
                    valKINV, vecDVEHVCRD(idxvehrotor==1), vecN(vecPANELROTOR>0), vecM(vecPANELROTOR>0), vecDVELE(idxvehrotor==1), vecDVEPANEL(idxvehrotor==1), vecAIRFOIL, vecDVENFREE(idxvehrotor==1)+vecDVENIND(idxvehrotor==1), vecDVEAREA(idxvehrotor==1),matUINF(idxvehrotor==1,:), matVLST, matDVE(idxvehrotor==1,:), matWUINF);
            end 
        end
    end 
end
