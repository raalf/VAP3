function [out, dataout] = fcnOBJECTIVE2(z, home_dir, views)
cd(home_dir)
...
if views == true
    ...
    zlim([0.3 0.9])
    view([0 0])
    saveFig2Latex(gcf, 'split-xz.pdf', wh);
    ...
    zlim([0.3 0.9])
    view([0 90])
    saveFig2Latex(gcf, 'split-xy.pdf', wh);
    % YZ
    gca;
    xlim([0.2 0.8])
    ylim([7 7.6])
    zlim([0.3 0.9])
    view([-90 0])
    saveFig2Latex(gcf, 'split-yz.pdf', wh);
end
