...
% cd ./../
% [baseline_out, baseline_ITER, baseline_ITEROUTP] = fcnBASELINE_OBJ();
% cd Analysis/
% cd ./../
% parfor i = 1:size(z,1)
...
%     baseline_ITEROUTP(end).OUTP.vecCDI_AVG; ...
%     baseline_ITEROUTP(end).OUTP.vecCDP_AVG];
% 
...
% 
% baseline = [baseline_ITEROUTP(end).OUTP.vecCP_AVG; ...
%     baseline_ITEROUTP(end).OUTP.vecCPI_AVG; ...
%     baseline_ITEROUTP(end).OUTP.vecCPP_AVG];
% 
...
clf(205);
loc = 2.*(Design(1).ITEROUTP(end).OUTP.WING(1).vecSPANLOC)./10;
drag = baseline_ITEROUTP(end).OUTP.WING(1).vecDIDIST_AVG(11:end) + baseline_ITEROUTP(end).OUTP.WING(1).vecDPDIST_AVG;
p = plot(loc, drag);
p.LineStyle = linestyles{1,:};
p.Marker = markers{1,:};
p.Color = colors{1,:};
