...
xsi(:,:,1) = x(:,:,1).*reshape(cpsi.*ceps,A,B) + x(:,:,2).*reshape(cpsi.*seps.*snu+spsi.*cnu,A,B) + x(:,:,3).*reshape(-cpsi.*seps.*cnu+spsi.*snu,A,B);
xsi(:,:,2) = x(:,:,1).*reshape(-spsi.*ceps,A,B) + x(:,:,2).*reshape(-spsi.*seps.*snu+cpsi.*cnu,A,B) + x(:,:,3).*reshape(spsi.*seps.*cnu+cpsi.*snu,A,B);
xsi(:,:,3) = x(:,:,1).*reshape(seps,A,B) + x(:,:,2).*reshape(-ceps.*snu,A,B) + x(:,:,3).*reshape(ceps.*cnu,A,B);
