...
idxtail = SURF.vecWINGTYPE == 2; % DVEs that are on the horizontal tail

tempVLST = SURF.matVLST(unique(SURF.matDVE(idxtail,:)),:) - repmat(SURF.matVLST(SURF.matDVE(SURF.idxTAIL(1),1),:), length(SURF.matVLST(unique(SURF.matDVE(idxtail,:)),1)),1);
tempCENTER = SURF.matCENTER(idxtail,:) - repmat(SURF.matVLST(SURF.matDVE(SURF.idxTAIL(1),1),:), length(SURF.matCENTER(idxtail,1)),1);
tempNPVLST = SURF.matNPVLST(unique(SURF.matNPDVE(idxtail,:)),:) - repmat(SURF.matNPVLST(SURF.matNPDVE(SURF.idxTAIL(1),1),:), length(SURF.matNPVLST(unique(SURF.matNPDVE(idxtail,:)),1)),1);
...
vlst2 = (ROT*tempVLST')' + repmat(SURF.matVLST(SURF.matDVE(SURF.idxTAIL(1),1),:), length(SURF.matVLST(unique(SURF.matDVE(idxtail,:)),1)),1) ;
npvlst2 = (ROT*tempNPVLST')' + repmat(SURF.matNPVLST(SURF.matNPDVE(SURF.idxTAIL(1),1),:), length(SURF.matNPVLST(unique(SURF.matNPDVE(idxtail,:)),1)),1);
center2 = (ROT*tempCENTER')' + repmat(SURF.matVLST(SURF.matDVE(SURF.idxTAIL(1),1),:), length(SURF.matCENTER(idxtail,1)),1);
...
SURF.vecTAILPITCHold = SURF.vecDVEPITCH(SURF.vecWINGTYPE == 2)-COND.vecVEHALPHA(i)*pi/180;
